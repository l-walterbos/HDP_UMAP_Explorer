...
dplot['Icohp.total_dir_asym'] = dinfo['Icohp.B1.directional_asym_index'] + dinfo['Icohp.B2.directional_asym_index'].fillna(0)
dplot['Icobi.total_dir_asym'] = dinfo['Icobi.B1.directional_asym_index'] + dinfo['Icobi.B2.directional_asym_index'].fillna(0)
dplot['Icohp.total_axial_asym'] = dinfo['Icohp.B1.axial_asym_index'] + dinfo['Icohp.B2.axial_asym_index'].fillna(0)
dplot['Icobi.total_axial_asym'] = dinfo['Icobi.B1.axial_asym_index'] + dinfo['Icobi.B2.axial_asym_index'].fillna(0)
dplot['Octh. Ratio'] = dplot.apply(lambda row: row['size_Oh_B1']/row['size_Oh_B2'] if row['size_Oh_B2']>row['size_Oh_B1'] else row['size_Oh_B2']/row['size_Oh_B1'],axis=1)
...
app.layout = html.Div([
    html.Div([
        html.Div([
            ...
            dcc.Dropdown(list(dumap.columns.get_level_values(0).unique()),value=list(dumap.columns.get_level_values(0).unique())[0],  id='umap-proj-data-dropdown',style={'width':'70%'}),
            html.Dt('Select metric:'),
            dcc.RadioItems(list(dumap.columns.get_level_values(1).unique()),value=list(dumap.columns.get_level_values(1).unique())[-1],id='umap-proj-metric-radio',inline=True),
            html.Dt('Select NN value (min_dist=0.1)'),
            html.Div([dcc.Slider(min=0,max=list(dumap.columns.get_level_values(2).astype(int).unique())[-1], step = None, marks={n: n for n in list(dumap.columns.get_level_values(2).unique())}, value=list(dumap.columns.get_level_values(2).astype(int))[0],id='umap-proj-nn-slider')],
        ...
        html.Div([
            ...
            dcc.Input(type='text',id='bsite_filter_input',placeholder='Select B-sites, e.g., "Ag,Bi,Au"...'),
            html.Dt('Filter: restrict B-sites to:'),
            dcc.Input(type='text',id='bsite_restricter_input',placeholder='Select B-sites, e.g., "Ag,Bi,Au"...'),
            html.Dt('Select X-sites:'),
            dcc.Checklist(['F','Cl','Br','I'],['F','Cl','Br','I'],id='xsite_checklist',inline=True)
    ...
    html.Div([
        html.Div([
            dcc.Graph(
                id='UMAP-plot',responsive=True,
                # figure=umapfig,
                clickData={'points': [{'hovertext': '1000_CsAgLuCl'}]}
            ),
        ], style={'width': '55%', 'height':'650px','display': 'inline-block'}),

        html.Div([
            html.Div([
                html.Dt('UMAP Color Options:'),
                dcc.Dropdown(options = dplot.columns.to_list(), value='bandgap', id='color-selector',style={'width':'80%'}),
                dcc.RadioItems(['Linear','Log'],'Linear',id='color-scaling-radio', inline=True,),
                html.Dt('Overwrite colorscale range'),
                html.Div([html.Div([dcc.Input(type='number',placeholder='minimal val.',step=0.001,style={'width':'90%'},id='range_scale_min_input')],style={'width':'50%','display':'inline-block'}),html.Div([dcc.Input(type='number',placeholder='maximal val.',step=0.001,style={'width':'90%'},id='range_scale_max_input')],style={'width':'50%','display':'inline-block'})]),
            ],style = {'width':'50%','display':'inline-block'}),
            
            html.Div([
                html.Dt('Ptable Color Options:' ,id='ptable-output'),
                dcc.Dropdown(ptable_color_list,value = ptable_color_list[0],id='ptable_color_dropdown',style={'width':'80%'}),
                dcc.RadioItems(['Linear','Log'],'Linear', id='ptable_scaling_radio',inline=True)
            ],style={'width':'50%','display':'inline-block'}),

            dcc.Graph(id='ptable-plot',responsive=True,style={'height':'500px'}),
            html.Dd('____')
        ],style={'width': '45%','display':'inline-block','height':'650px'}),
    ],style={'width':'100%'}),

    html.Div([
        html.Div([
            html.Dt('Select Bonding Plot:'),
            dcc.Dropdown(['COHP','COBI'],value='COHP',id='coxx_selector'),
            html.Dd('_____'),
            html.Dt(id='disc-comp-name'), html.Dd(id='disc-bandgap'), html.Dd(id='disc-totalspin'), html.Dd(id='disc-b1spin'), html.Dd(id='disc-b2spin'),
            html.Dt('Loewdin Charges'), html.Dd(id='disc-chargeA'), html.Dd(id='disc-chargeB1'), html.Dd(id='disc-chargeB2'), html.Dd(id='disc-chargeX'), 
            html.Dt('Bonding Data'), html.Dd(id='disc-icohpB1'), html.Dd(id='disc-icohpB2'), html.Dd(id='disc-icobiB1'), html.Dd(id='disc-icobiB2')
...
def update_sidegraphs(selected_point,selected_coxx):
    ...
    coxxfigs = plot_coxx(selected_comp, info_df_path, coxx_type=selected_coxx.lower(),
...
def update_compdiscription(selected_point):
    ...
    elecdatastring = [f'Bandgap: {compseries['bandgap']:.2f} eV',f'Net.Spin: {compseries['popdiff.total']}', f'Net.Spin[B1]: {compseries['popdiff.B1']}',f'Net.Spin[B2]: {compseries['popdiff.B2']}']
    chargestrings = [f'Charge A: {compseries['charge.A']}',f'Charge B1: {compseries['charge.B1']}', f'Charge B2: {compseries['charge.B2']}',f'Charge X: {round(compseries['charge.X'],2)}',]
    coxxstrings = [f'ICOHP B1-X: {compseries['Icohp.B1.avg']:.2f} eV',f'ICOHP B2-X: {compseries['Icohp.B2.avg']:.2f} eV',f'ICOBI B1-X: {compseries['Icobi.B1.avg']:.2f}',f'ICOBI B2-X: {compseries['Icobi.B2.avg']:.2f}',]
...
def change_color_umap(selected_column,color_scale,selected_data, selected_metric, nn_value, bsite_filter, bsite_restricter, xsite_filter, min_range_val, max_range_val):
    ...
    if len(bfilt_input_string) == 0 or bfilt_input_string[0] == 'None' or bfilt_input_string[0] == '':
        belement_list = set(list(dinfo['element.B1'].unique()) + list(dinfo['element.B2'].unique()))
    else:
        belement_list = [x.strip().capitalize() for x in bfilt_input_string]
    # print(belement_list)
    # print(restrict)
    if restrict:
        bfiltered_index = list(dinfo[(dinfo['element.B1'].isin(belement_list)) & dinfo['element.B2'].isin(belement_list)].index)
    else:
    ...
    if dplot[selected_column].dtype == np.float64:
        colorcolumn = dplot[selected_column] if not color_scale == 'Log' else np.log10(np.abs(dplot[selected_column]) )
        ...
        if color_scale == 'Log':
            ...
            orig_min = np.log10(min_range_val) if not np.isnan(min_range_val) else colorcolumn[~colorcolumn.isin([np.nan,-np.inf, np.inf])].min()
            orig_max = np.log10(max_range_val) if not np.isnan(max_range_val) else colorcolumn[~colorcolumn.isin([np.nan,-np.inf, np.inf])].max()

            # orig_min = minrange
            # orig_max = maxrange
            # print(orig_min,orig_max,)
            # tick_values = np.logspace(np.log10(orig_min), np.log10(orig_max), num=10, endpoint=True)
            tick_values1 = list(np.linspace(10**orig_min, 10**(orig_max/3), num=4, endpoint=True))
            tick_values2 = list(np.linspace(10**(orig_max/3), 10**(2*orig_max/3), num=3, endpoint=True))
            tick_values3 = list(np.linspace(10**(2*orig_max/3), 10**orig_max, num=3, endpoint=True))
            ...
            orig_min = min_range_val if not np.isnan(min_range_val) else np.floor(colorcolumn.dropna().min())
            orig_max = max_range_val if not np.isnan(max_range_val) else np.ceil(colorcolumn.dropna().max())
    ...
    umapfig = px.scatter(dfilt, x='x',y='y',
                         color=colorcolumn.loc[filtered_index],
                         color_continuous_scale='Plasma',
                         color_discrete_map=discretecolormap,
                         category_orders= {'transition_bands':list(discrete_colors_bands.keys()),
    ...
    umapfig.add_trace(go.Scatter(x=dnegative['x'],y=dnegative['y'],mode='markers',marker={'color':'grey','opacity':0.4,'size':3.5},showlegend=False,hovertext=dnegative.index.to_list()))
    umapfig.update_layout(xaxis_title='UMAP 1', xaxis=dict(title=dict(font=dict(size=17))))
    umapfig.update_xaxes(showticklabels=False)
    umapfig.update_layout(yaxis_title='UMAP 2', yaxis=dict(title=dict(font=dict(size=17))))
    ...
    umapfig.update_layout(clickmode='event', legend=dict(font=dict(size=18), title=dict(font=dict(size=18))), coloraxis_colorbar=dict(title=colorcolumn.name,tickfont=dict(size=18),**colorbar))
    return umapfig
