...
dos_output_dir = Path('/home/lwalterb/hdp_project/umap_interactive/smeared_histogrammed_doss')
cohp_output_dir = Path("/home/lwalterb/hdp_project/umap_interactive/smeared_histogrammed_cohps")
...
normcohpB1avg_up = pd.read_csv(cohp_output_dir / 'norm_hist_cohp_B1avg_up.csv',index_col=0)
normcohpB1avg_down = pd.read_csv(cohp_output_dir / 'norm_hist_cohp_B1avg_down.csv',index_col=0)
normcohpB1avg_tot = pd.concat([normcohpB1avg_up,normcohpB1avg_down],ignore_index=True).T

normcohpB2avg_up = pd.read_csv(cohp_output_dir / 'norm_hist_cohp_B2avg_up.csv',index_col=0)
normcohpB2avg_down = pd.read_csv(cohp_output_dir / 'norm_hist_cohp_B2avg_down.csv',index_col=0)
normcohpB2avg_tot = pd.concat([normcohpB2avg_up,normcohpB2avg_down],ignore_index=True).T




# print(dosTtot)
def create_all_projections(data_dictionary:dict, metric_names:list,NN_list:list, min_dist:float = 0.1, connector_symbol:Literal['+','*','-']='*'):
    ...
    for dataname, metric in product(datnames,metric_names):
        ...
        if type(data_dictionary[dataname])==list and len(data_dictionary[dataname]) == 2:
            if metric == 'seuclidean':
                ...
                metric_kwd1 = {'sigma': np.array(data_dictionary[dataname][0].std(),dtype=np.float64)+np.ones(data_dictionary[dataname][0].shape[1])*0.0001}
                metric_kwd2 = {'sigma': np.array(data_dictionary[dataname][1].std(),dtype=np.float64)+np.ones(data_dictionary[dataname][1].shape[1])*0.0001}
                # print(metric_kwd['sigma'].shape)
            elif metric == 'wminkowski':    
                metric_kwd1 = {'sigma': 1/(np.array(data_dictionary[dataname][0].std(),dtype=np.float64)+np.ones(data_dictionary[dataname][0].shape[1])*0.0001),'p':1}
                metric_kwd2 = {'sigma': 1/(np.array(data_dictionary[dataname][1].std(),dtype=np.float64)+np.ones(data_dictionary[dataname][1].shape[1])*0.0001),'p':1}
            else:
                metric_kwd1 = {}
                metric_kwd2 = {}
            #precompute highest number of k-NN to speed up multiple projections
            knn1 = nearest_neighbors(data_dictionary[dataname][0], n_neighbors=max(NN_list), metric=metric, metric_kwds=metric_kwd1, angular=False, random_state=None)
            knn2 = nearest_neighbors(data_dictionary[dataname][1], n_neighbors=max(NN_list), metric=metric, metric_kwds=metric_kwd2, angular=False, random_state=None)
        elif type(data_dictionary[dataname])==list and len(data_dictionary[dataname]) == 4:
            if metric == 'seuclidean':
                #calculate standard deviations to put in as weights for standardised euclidean
                #add 1e-4 to each std to compensate for 20 highest energy points being empty
                metric_kwd1 = {'sigma': np.array(data_dictionary[dataname][0].std(),dtype=np.float64)+np.ones(data_dictionary[dataname][0].shape[1])*0.0001}
                metric_kwd2 = {'sigma': np.array(data_dictionary[dataname][1].std(),dtype=np.float64)+np.ones(data_dictionary[dataname][1].shape[1])*0.0001}
                metric_kwd3 = {'sigma': np.array(data_dictionary[dataname][2].std(),dtype=np.float64)+np.ones(data_dictionary[dataname][2].shape[1])*0.0001}
                metric_kwd4 = {'sigma': np.array(data_dictionary[dataname][3].std(),dtype=np.float64)+np.ones(data_dictionary[dataname][3].shape[1])*0.0001}
                # print(metric_kwd['sigma'].shape)
            elif metric == 'wminkowski':    
                metric_kwd1 = {'sigma': 1/(np.array(data_dictionary[dataname][0].std(),dtype=np.float64)+np.ones(data_dictionary[dataname][0].shape[1])*0.0001),'p':1}
                metric_kwd2 = {'sigma': 1/(np.array(data_dictionary[dataname][1].std(),dtype=np.float64)+np.ones(data_dictionary[dataname][1].shape[1])*0.0001),'p':1}
                metric_kwd3 = {'sigma': 1/(np.array(data_dictionary[dataname][2].std(),dtype=np.float64)+np.ones(data_dictionary[dataname][2].shape[1])*0.0001),'p':1}
                metric_kwd4 = {'sigma': 1/(np.array(data_dictionary[dataname][3].std(),dtype=np.float64)+np.ones(data_dictionary[dataname][3].shape[1])*0.0001),'p':1}
            else:
                metric_kwd1 = {}
                metric_kwd2 = {}
                metric_kwd3 = {}
            ...
            knn1 = nearest_neighbors(data_dictionary[dataname][0], n_neighbors=max(NN_list), metric=metric, metric_kwds=metric_kwd1, angular=False, random_state=None)
            knn2 = nearest_neighbors(data_dictionary[dataname][1], n_neighbors=max(NN_list), metric=metric, metric_kwds=metric_kwd2, angular=False, random_state=None)
            knn3 = nearest_neighbors(data_dictionary[dataname][2], n_neighbors=max(NN_list), metric=metric, metric_kwds=metric_kwd3, angular=False, random_state=None)
            knn4 = nearest_neighbors(data_dictionary[dataname][3], n_neighbors=max(NN_list), metric=metric, metric_kwds=metric_kwd4, angular=False, random_state=None)
        else:
            if metric == 'seuclidean':
                #calculate standard deviations to put in as weights for standardised euclidean
                #add 1e-4 to each std to compensate for 20 highest energy points being empty
                metric_kwd = {'sigma': np.array(data_dictionary[dataname].std(),dtype=np.float64)+np.ones(data_dictionary[dataname].shape[1])*0.0001}
                print(metric_kwd['sigma'].shape)
            elif metric == 'wminkowski':    
                metric_kwd = {'sigma': 1/(np.array(data_dictionary[dataname].std(),dtype=np.float64)+np.ones(data_dictionary[dataname].shape[1])*0.0001),'p':1}
            else:
                metric_kwd = {}
            knn = nearest_neighbors(data_dictionary[dataname], n_neighbors=max(NN_list), metric=metric, metric_kwds=metric_kwd, angular=False, random_state=None)

        for nn_val in NN_list:

            print(f'working on projection: {dataname},{metric},{nn_val}')
            if type(data_dictionary[dataname])==list and len(data_dictionary[dataname]) == 2:
                reducer1 = umap.UMAP(n_neighbors=nn_val, min_dist=min_dist, precomputed_knn=knn1)
                reducer2 = umap.UMAP(n_neighbors=nn_val, min_dist=min_dist, precomputed_knn=knn2)
                ...
                reducer1 = umap.UMAP(n_neighbors=nn_val, min_dist=min_dist, precomputed_knn=knn1)
                reducer2 = umap.UMAP(n_neighbors=nn_val, min_dist=min_dist, precomputed_knn=knn2)
                reducer3 = umap.UMAP(n_neighbors=nn_val, min_dist=min_dist, precomputed_knn=knn3)
                reducer4 = umap.UMAP(n_neighbors=nn_val, min_dist=min_dist, precomputed_knn=knn4)
                proj1 = reducer1.fit(data_dictionary[dataname][0])
                proj2 = reducer2.fit(data_dictionary[dataname][1])
                ...
                reducer = umap.UMAP(n_neighbors=nn_val, min_dist=min_dist, precomputed_knn=knn)
